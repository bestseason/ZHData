...
public class Answer2Activity extends Activity {

    Handler handler=new Handler(){
        public void handleMessage(Message message){
            if(message.what==1){
                Toast.makeText(Answer2Activity.this, "提交成功", Toast.LENGTH_SHORT).show();
                Intent intent1 = new Intent(Answer2Activity.this,Question_page_Activity.class);
                intent1.putExtra("Qid",Qid+"");
                startActivity(intent1);
                finish();
            }
            else if(message.what==2)
                Toast.makeText(Answer2Activity.this, "匿名用户不能回答问题请先登录", Toast.LENGTH_SHORT).show();
            else if(message.what==3)
                Toast.makeText(Answer2Activity.this, "提交失败", Toast.LENGTH_SHORT).show();
            else if(message.what==4)
                Toast.makeText(Answer2Activity.this, "回答内容不能为空", Toast.LENGTH_SHORT).show();
            else if(message.what==5){
//                icarus.insertHtml("<img src=\"http://182.254.216.178:8080"+message.obj+"\" style=\"width:100%\">");
                icarus.insertHtml("<img src=\""+BaseNetwork.IP_ADDRESS.substring(0,BaseNetwork.IP_ADDRESS.length()-1)+message.obj+"\" style=\"width:100%\">");
            }else if(message.what==6){
                progress=100;
                progressDialog.setProgress(100);
                handler.sendEmptyMessageDelayed(10, 100);
                Toast.makeText(Answer2Activity.this, "图片上传成功", Toast.LENGTH_SHORT).show();
            }else if(message.what==7){
                progress=100;
                progressDialog.setProgress(100);
                handler.sendEmptyMessageDelayed(10, 100);
                Toast.makeText(Answer2Activity.this, "图片上传失败", Toast.LENGTH_SHORT).show();
...
//    private EditText et;
    ...
    public class myTextViewButton extends TextViewButton{
        private static final int REQUEST_IMAGE = 2;
        public myTextViewButton(TextView textView, Icarus icarus) {
        ...
        public void command() {
            ...
            MultiImageSelector selector =MultiImageSelector.create(Answer2Activity.this);
            selector.start(Answer2Activity.this, REQUEST_IMAGE);
        }
    }
...
//                options.setDefaultImage(path.get(0));
//                icarus.jsExec("javascript: editor.toolbar.execCommand('image')");
//                icarus.insertHtml("<img src=\"http://192.168.137.1:8080/images/163679-user.jpg\" style=\"width:100%\">");
                try{
                    final List<HashMap<String,Object>> list=new ArrayList<>();
                    for (int i=0;i<path.size();i++) {
                        Bitmap bitmap= BitmapFactory.decodeStream(this.getContentResolver().openInputStream(Uri.parse("file://"+path.get(i))));
                    ...
                    new Thread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            for (int j=0;j<list.size();j++) {
                                String address = baseNetwork.uploadFile("Aupload", Answer2Activity.this, (Bitmap)list.get(j).get("Bitmap"), (String)list.get(j).get("fileName"), Sid);
                                if (!address.equals("")) {
...
//                for (int i=0;i<path.size();i++) {
//                    icarus.insertHtml("<img src=\"" + path.get(i) + "\" style=\"width:100%\">");
//                }
...
//                });
    ...
    private Toolbar prepareToolbar(TextViewToolbar toolbar, Icarus icarus) {
        ...
        myTextViewButton imageButton = new myTextViewButton(imageButtonTextView, icarus);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        progressDialog=new ProgressDialog(Answer2Activity.this);
...
//        et=(EditText)findViewById(R.id.answer2_edit);
        Intent intent=getIntent();
//        if(intent.getStringExtra("Sid").equals("")==false) {
...
//        }
        ...
        Qid=intent.getIntExtra("Qid",0);
         ...
         date = new Date(System.currentTimeMillis());//获取系统当前日期

    }
    public void click(View view){
        switch (view.getId()){
            case R.id.answer2_img1://返回到回答1界面
                finish();
                break;
            case R.id.answer2_img2://提交回答
                icarus.getContent(new Callback() {
                    @Override
                    public void run(String params) {
                        Gson gson = new Gson();
                        ...
                        if(Sid.equals("-1")==true){
                            ...
                            if (!text.equals("")) {
                                ...
                                boolean is = baseNetwork1.addAnswer(Sid, Qid, HtmlUtil.replaceUploadHtmlTag(text), date);
                                Message ms = new Message();
                                if (is == true) {
                                    ms.what = 1;
                                } else {
